...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if ocr_data_path is not None:
            ocr_data_list = open(ocr_data_path)
            ocr_data = list(ocr_data_list.readlines())
            ocr_data_dict = self.data_preprocess(ocr_data, ocr_image_path)
            list_data_dict += ocr_data_dict * 2

        if textvqa_data_path is not None:
            text_data_list = open(textvqa_data_path)
            text_data = list(text_data_list.readlines())
